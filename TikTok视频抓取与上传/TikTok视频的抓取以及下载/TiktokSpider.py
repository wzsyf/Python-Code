    ...
    def getMsgsAndPersist(self):
        ...
        for tiktokUrl in TikTokUrls:
            ...
            if not os.path.exists(os.path.join(rootDir, uniqueId)):
                ...
                # 用户信息
                authorInfos = {
                    'nickname': items[0]['author']['nickname'],
                    'uid': uniqueId,
                    'avatar': items[0]['author']['avatarMedium']
                }
            ...
            for item in items:
                if item['stats']['diggCount'] >= 10000:

                    video_msg = item['video']

                    createTime = str(time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime(item['createTime'])))
