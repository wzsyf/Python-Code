...
class Backend:
    ...
    def disable_brand(self):
        sql = "select * from sylius_brand where parent_id=2 order by id asc"
        res = self.conn.execute(sql)
        rows = res.fetchall()
        for row in rows:
            taxonId = row["id"]
            sql = "select count(*) as total from sylius_product and enabled=1 and id in (select product_id from sylius_taxon where taxon_id={0})".format(taxonId)
            res = self.conn.execute(sql)
            _row = res.fetchone()
            if _row:
                total = _row["total"]
                if total == 0:
                    sql = "update sylius_taxon set status=1 where id = {0}".format(taxonId)
                    self.conn.execute(sql)

    def short_url_for_blogs(self):
        sql = "select id, code from mukhin_sylius_blogging_blog where short_url is null or short_url ='' order by id desc limit 50"
        res = self.conn.execute(sql)
        rows = res.fetchall()
        for row in rows:
            blogId = row["id"]
            ...
            if shortUrl:
                sql = "update mukhin_sylius_blogging_blog set short_url = '{0}' where id = {1}".format(shortUrl, blogId)
                self.conn.execute(sql)
    ...
    def htmlParser(self):
        ...
        sql = "select product_id,brand_name,product_name,description from productdata"
        res = self.conn_import.execute(sql)
        rows = res.fetchall()
        for row in rows:
            productId = row["product_id"]
            brandName = repr(str(parser.unescape(row["brand_name"])))
            productName = repr(str(parser.unescape(row["product_name"])))

            update = """update productdata set brand_name = {0}, product_name = {1} where product_id = "{2}";""".format(brandName, productName, productId)
            self.conn_import.execute(update)



    def analysisProductCategory(self, category):
        map = {}

        sql = "select count(sylius_product.id) num from sylius_product inner join (select id from sylius_taxon where code like '%%{0}%%') a on sylius_product.main_taxon_id = a.id where enabled = 1;".format(category)
        res = self.conn.execute(sql)
        rows = res.fetchall()
        for row in rows:
            num = row["num"]
    ...
    def getIGAccountEmail(self, igAccount):
        ...
        sql = "select email from sylius_customer where id in (select * from (select id from sylius_customer where instagram_account = '{0}') a);".format(igAccount)
        res = self.conn.execute(sql)
        rows = res.fetchall()
    ...
    def products_need_disable(self):
        ...
        sql = "select original_id,flags from sylius_product where {0} and enabled=1".format(clause)
        print sql
        res = self.conn.execute(sql)
        rows = res.fetchall()
        ids = []
    ...
    def all_products(self, stores=None):
        ...
        #sql = "select product_id as id from sylius_product_taxon where taxon_id = 11"
        res = self.conn.execute(sql)
        rows = res.fetchall()
        for row in rows:
            item = {}
    ...
    def all_enabled_products(self, stores=None):
        ps = []
        sql = "select * from sylius_product where order by id asc"
        if stores:
            stores = "','".join(stores)
            stores = "'{0}'".format(stores)
