        ...
        for i in range(0, columnLen - 1):
            for j in range(i + 1, columnLen):
                ...
                if subStrLen >= 6:
                    ...
                    # commonSubStr.append(column[i])
                    # commonSubStr.append(column[j])

        return allCommonSubstr


    # def removeRepeat(self, dic):
    #     haveNotRepeat = {}
    #
    #     for ele in dic.keys():
    #         if ele not in haveNotRepeat.keys():
    #             haveNotRepeat[ele] = dic.get(ele)
    #
    #     return haveNotRepeat
